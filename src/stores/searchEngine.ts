...
export const useSearchEngineStore = defineStore("searchEngine", () => {
  const searchEngines = ref<SearchEngine[]>([
    {
      ...
      title: "DuckDuckGo",
      suffix: "d",
      querystring: "https://duckduckgo.com/?q=%s",
      isDefault: true,
    },
    {
      id: 2,
      title: "Youtube",
      suffix: "y",
      querystring: "https://www.youtube.com/results?search_query=%s",
      isDefault: false,
    },
  ]);
  ...
  function getEngineBySuffix(suffix: string) {
    return (
      searchEngines.value.find((se) => se.suffix === suffix) ??
  ...
  function buildUrl(id: number, query: string): string {
    ...
    if (!engine.querystring.includes("%s"))
      throw new Error("Query template must contain %s");
